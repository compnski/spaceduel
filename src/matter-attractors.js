  ...
  Engine: {
    ...
    update: function update(engine) {
      var world = engine.world,
		  ...
		  delta = engine.timing.timestamp - MatterAttractors.Engine.previousTimestamp;
		MatterAttractors.Engine.previousTimestamp = engine.timing.timestamp;		
          ...
          for (var j = i + 1; j < bodies.length; j += 1) {
            ...
            for (var k = 0; k < attractors.length; k += 1) {
              ...
              if (Matter.Common.isFunction(attractor)) {
                forceVector = attractor(bodyA, bodyB, delta);
              }
